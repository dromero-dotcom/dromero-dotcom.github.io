...
The datasets included 11,879 reviews from Google and 16,581 from Trustpilot (in English), spanning March 2022 to May 2024. The analysis identified 310 locations common in both sets, pin-pointing the "top 20
...
* Initial Topic Modelling: Used BERTopic (a modular, transformer-based topic modelling library ) to identify topic clusters within the negative reviews.
<img src="images/proj3/Bertopic_All_Negatives.png?raw=true"/>

* Top 30 Locations Analysis: Aggregated the review counts to find the top 30 locations with highest number of negative reviews. Applied BERTopic to this subset.
* Emotion Analysis: Used a BERT-based model from Hugging-Face to extract the top “emotion” from each negative review. Subsequently, reviews characterized by "anger" were isolated.
<img src="images/proj3/Emotions_Distribution.png?raw=true"/>

* Deep-dive into "angry" reviews: BERTopic was then applied to the subset of "Angry Reviews" to understand specific drivers of high-intensity dissatisfaction.
...
* Latent Dirichlet Allocation (LDA) Modelling: used the LDA model from Gensim to perform topic modelling on the negative reviews (all locations) to provide a comparative baseline.
...
A map showing the top 20 sites with highest number of negative reviews was generated, so these could be prioritised for further investigation and targetd action. 
For data protection reasons, the actual map is not shown here, but an example (unrelated) of the visualisation is below:
<img src="images/proj3/Map_Example.jpg?raw=true"/>
